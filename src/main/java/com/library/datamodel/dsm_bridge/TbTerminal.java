...
@NamedQueries({
    @NamedQuery(name = TbTerminal.FETCH_TERMINAL, query = TbTerminal.FETCH_TERMINAL_QUERY)
})
public class TbTerminal implements Serializable, DBInterface {

    public static final String FETCH_TERMINAL_QUERY = "SELECT DISTINCT terminal FROM TbTerminal terminal INNER JOIN terminal.id terminId INNER JOIN terminal.tbLoopTask loopTask where terminId.devId=:devId and terminId.cstmId=cstmId";
